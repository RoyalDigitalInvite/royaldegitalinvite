...
function showScene2() {
  ...
  // Le chandelier apparaît seulement après la disparition complète
  // de l'enveloppe
  setTimeout(() => {
    document.body.classList.add("chandelier-active");
  }, 1700);
}
...
function startIntro() {
  ...
  startMusic();
  activateFloatingLogo();
  scene1.classList.add("is-opening");   /* ← déclenche swingTap sur le chandelier */
  setTimeout(() => {
    showScene2();
  }, 1700);
}
...
if (rsvpForm) {
  ...
  rsvpForm.addEventListener("submit", (e) => {
    ...
    const guests = guestsField ? guestsField.value : "1";
    ...
    if (attendance === "yes" && (!guests || guests === "0")) {
      showFieldError(guestsField, "Merci de sélectionner le nombre d'invités.");
      if (!hasError && guestsField) guestsField.focus();
      hasError = true;
    }
